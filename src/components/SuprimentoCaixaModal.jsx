...
const SuprimentoCaixaModal = ({ isOpen, onClose, caixaId, cashierName, currentBalance, onSuccess }) => {
    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[60] p-4 backdrop-blur-sm animate-in fade-in duration-200">
      ...
      >
        ...
        <div className="px-6 py-4 bg-[#232f3e]">
           <span className="text-gray-400 text-xs uppercase block mb-1">Saldo Atual</span>
           <span className="text-2xl font-bold text-white">R$ {parseFloat(currentBalance || 0).toFixed(2)}</span>
        </div>
